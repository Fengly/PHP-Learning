...
class UserController extends HomeController {
	...
	public function register($username = '', $password = '', $repassword = '', $email = '', $mobile = ''){
		...
		if(IS_POST){ //注册用户
			...
			if(0 < $uid){ //注册成功
				//TODO: 发送验证邮件
                $list = M('ucenter_member')->field('id,username')->where(['id'=>$uid])->find();
             ...
             return $this->ajaxReturn($this->showRegError($uid),'json');
               // return $this->ajaxReturn($username,'json');
			}
		...
		if(IS_POST){ //登录验证
			...
			if(0 < $uid){ //UC登录成功
				...
				if($Member->login($uid)){ //登录用户
					...
					$this->ajaxReturn($Member->getError(),'JSON');
				}
