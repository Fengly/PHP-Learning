...
(function($, window) {
  ...
  ResizableColumns = (function() {
    ...
    ResizableColumns.prototype.createHandles = function() {
      ...
      this.$table.find('tr td').each(function(i, el) {
        var $handle;

        if (_this.$table.find('tr td').eq(i + 1).length === 0 || (_this.$table.find('tr td').eq(i).attr('data-noresize') != null) || (_this.$table.find('tr td').eq(i + 1).attr('data-noresize') != null)) {
          return;
        }
    ...
    ResizableColumns.prototype.syncHandleWidths = function() {
      ...
      return this.$handleContainer.find('.rc-handle').each(function(_, el) {
        return $(el).css({
          left: $(el).data('td').outerWidth() + ($(el).data('td').offset().left - _this.$handleContainer.offset().left),
    ...
    ResizableColumns.prototype.saveColumnWidths = function() {
      ...
      return this.$table.find('tr td').each(function(_, el) {
        var id;
    ...
    ResizableColumns.prototype.mousedown = function(e) {
      ...
      $leftColumn = $currentGrip.data('td');
      leftColumnStartWidth = $leftColumn.width();
      idx = this.$table.find('tr td').index($currentGrip.data('td'));
      $rightColumn = this.$table.find('tr td').eq(idx + 1);
