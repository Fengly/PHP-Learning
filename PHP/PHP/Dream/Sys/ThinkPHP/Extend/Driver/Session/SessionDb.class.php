...
class SessionDb {
   ...
   protected $hander  = array(); 
    ...
    public function open($savePath, $sessName) { 
	   $conn = require(CONF_PATH.'conn.php');
	   $cfg = require(CONF_PATH.'appcfg.php');
       $this->lifeTime = C('SESSION_EXPIRE')?C('SESSION_EXPIRE'):ini_get('session.gc_maxlifetime');
       $this->sessionTable  =   C('SESSION_TABLE')?C('SESSION_TABLE'):C("DB_PREFIX")."session";
       //分布式数据库
       $host = explode(',',$conn['DB_HOST']);
       $port = explode(',',$conn['DB_PORT']);
       $name = explode(',',$conn['DB_NAME']);
       $user = explode(',',$conn['DB_USER']);
       $pwd  = explode(',',$conn['DB_PWD']);
   ...
   public function write($sessID,$sessData) {
       ...
       $re = mysql_query("REPLACE INTO  ".$this->sessionTable." (  session_id, session_expire, session_data)  VALUES( '$sessID', '$expire',  '$sessData')",$hander); 
	  // dump($this->hander);exit;
