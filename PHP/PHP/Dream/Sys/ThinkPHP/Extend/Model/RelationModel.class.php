                    ...
                    if(empty($name) || true === $name || $mappingName == $name || (is_array($name) && in_array($mappingName,$name))) {
                        ...
                        switch($mappingType) {
                            ...
                            case MANY_TO_MANY:
								...
								//$this->relationInfo = $relationData;
                                if (!empty($val['relation_deep'])){
                                    foreach($relationData as $key=>$data){                                    
                                        $model->getRelation($data,$val['relation_deep']);
                                        $relationData[$key]     =   $data;
 ...
 */
                                        ...
                                        //修改 start
										$rs = M();
										$sql = "SELECT value_id FROM ".C('DB_PREFIX')."{$mappingRelationTable} where `".$mappingFk."`=".$mappingCondition[$mappingFk];
										$result = $model->where("id in(".$sql.")")->delete();
