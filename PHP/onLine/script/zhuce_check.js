function checkphone(lxdh){
	...
	var objTel = new RegExp(telephone);
	if(objExp.test(str)==true || objTel.test(str) == true){
		return true;
	}else{
		return false;
	}
...
//用户名   
	if (zhuce.zc_username.value == "" || zhuce.zc_username.value.length<3){
		alert("用户名不能为空且不能少于3个字符！");
